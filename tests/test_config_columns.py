...
def _aggregates(signal: str) -> list:
    return [
        f"{signal}_{stat}"
        for stat in ("mean", "std", "min", "max", "first", "last", "delta")
    ]
...
STEP_EXPECTED = [
    ...
    *_aggregates("current"),
    *_aggregates("potential"),
    *_aggregates("charge_capacity"),
    *_aggregates("discharge_capacity"),
    *_aggregates("power"),
    *_aggregates("charge_energy"),
    *_aggregates("discharge_energy"),
    *_aggregates("internal_resistance"),
