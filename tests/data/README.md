# Test data fixtures

Small, real cycling-data fixtures used by the test suite. They let the compute
engine be tested on genuine instrument output **without** depending on instrument
loaders (Arbin ODBC, Maccor/Neware parsers, …) — cellpy-core is the *core* engine
and stays loader-free.

Fixture filenames use a generic **`cycler`** prefix (not a vendor brand); the
underlying source files in the cellpy repo may still come from a specific tester.

## Files

| File | Source (cellpy repo) | Shape | Role |
|------|----------------------|-------|------|
| `cycler_cc_raw.parquet` | `testdata/data/20160805_test001_45_cc_01.res` | 10 261 rows, 18 cols, 18 cycles | Canonical golden oracle |
| `cycler_cc_steps_expected.parquet` | *(derived)* | 103 steps | Frozen snapshot of the current engine's step table |
| `cycler_cc_summary_expected.parquet` | *(derived)* | 18 cycles, 27 cols | Frozen snapshot of the current per-cycle summary |
| `cycler_cc_steptypes_cellpy.csv` | `testdata/data/steps.csv` | 103 steps, 4 cols | **Independent** cellpy step-type golden (cross-repo parity, Phase 4) |
| `cycler_small_raw.parquet` | `testdata/data/20200624_test001_cc_01.h5` | 47 rows, 1 cycle, 3 steps | Tiny fast fixture (no ODBC) |
| `cycler_cc_harmonized_raw.parquet` | *(derived from `cycler_cc_raw.parquet`)* | 10 261 rows, 28 cols, 18 cycles | Same data in the **harmonized** schema (`RawCols` naming) |

The original raw frames use the legacy `HeadersNormal` column naming
(`data_point`, `voltage`, `charge_capacity`, …) that the engine currently
consumes. `cycler_cc_harmonized_raw.parquet` is the one exception: it is the same
cycling data renamed into the **harmonized raw** schema
(`cellpycore.config.RawCols`: `datapoint_num`, `potential`,
`cumulative_charge_capacity`, …) — see
[`docs/specifications/harmonized-raw.md`](../../docs/specifications/harmonized-raw.md).

## Golden numbers

Mirrored from cellpy's own suite (`tests/test_cell_readers.py`) and verified to
be reproduced by cellpy-core's engine on `cycler_cc_raw.parquet`:

- step-table rows: **103**
- summary cycles: **18**
- cycle-1 last `data_point`: **1457**

`cycler_cc_steptypes_cellpy.csv` is a stronger, **per-step** cross-library check: it is
cellpy's own committed `steps.csv` golden (cycle/step/type/info), which predates the
cellpy→cellpy-core engine integration, so reproducing it byte-for-byte proves genuine
cross-repo parity rather than a self-snapshot.

## Provenance & license

Source files come from the [cellpy](https://github.com/jepegit/cellpy) repository
(battery test data originating from IFE, Norway). cellpy is MIT-licensed; these
small derived snapshots are vendored here under the same terms for testing only.

## Regenerating

Fixtures are produced by `dev/regenerate_test_data.py` in two stages (different
environments, because the engine working copy and the instrument loaders live in
different venvs):

```bash
# Stage A — raw export (needs cellpy + Arbin ODBC for the .res); run in cellpy's env
cd ../cellpy && uv run python ../cellpy-core/dev/regenerate_test_data.py

# Stage B — engine snapshot (needs the cellpy-core working copy); run in cellpy-core's env
cd ../cellpy-core && uv run python dev/regenerate_test_data.py
```

Set `CELLPY_REPO` if the cellpy checkout is not the sibling `../cellpy`.

Regenerate the `*_steps_expected.parquet` / `*_summary_expected.parquet` snapshots
**intentionally** (and review the diff) only when a step-table or summary change is
expected.

`cycler_cc_harmonized_raw.parquet` is produced separately (pure-polars, no cellpy
needed) by `dev/make_harmonized_raw.py`, which renames `cycler_cc_raw.parquet` into
the `RawCols` schema. Re-run it after any `RawCols` change so the fixture tracks
the schema:

```bash
uv run python dev/make_harmonized_raw.py
```

## curve_goldens/

Curve-extraction parity oracles for `cellpycore.curves` (issue #118), vendored
verbatim from cellpy's Stage-0 snapshots
(`cellpy/tests/data/goldens/curve_*/curve.parquet`, jepegit/cellpy#433; same
golden cell, mass = 1.0 mg, gravimetric). Regenerate on the cellpy side and
re-vendor — never edit by hand. Consumed by `tests/test_curves.py`.
