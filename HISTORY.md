...
- Rename the absolute nominal-capacity parameter to `nom_cap_abs` across the
  ...
  `specific_conversion_factors`). (#99)
- Rename `specific_converters` to `specific_conversion_factors` on
  `add_scaled_summary_columns` (and `calculate_specific_conversion_factors` in
  `cellpycore.units`); the old names are deprecated aliases. (#110)
- Add autogenerated API reference from docstrings via mkdocstrings
  (`docs/api/`, mkdocstrings plugin in `zensical.toml`). (#110)
- Add standalone unit-conversion helpers (`calculate_nom_cap_abs_from_specific`,
  `calculate_current_conversion_factor`, `calculate_specific_conversion_factors`)
  and expand the standalone-use unit documentation. (#110)
