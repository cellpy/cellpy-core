...
| c_rate | float | 1/h (C) | 0.05 | Per-step C-rate estimate (legacy `rate_avr`); not part of the base `make_step_table` output — appended by the separate opt-in `add_step_c_rate(data, nom_cap_abs=...)` (called by `make_core_step_table`) |